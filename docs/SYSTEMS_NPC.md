...
- **NPC chat backends**: `AI_BACKEND` supports `cli` (Claude CLI, default), `api` (Anthropic API), or `llamacpp` (local `llama-server` via [LLMFacade](https://github.com/Coamithra/LLMFacade)). `ollama` is accepted as a deprecated alias for `llamacpp`. The llamacpp backend uses LLMFacade's external mode; context size, KV-cache quantization, and parallelism are owned by the `llama-server` CLI, not by Python. Configure via `LLAMACPP_BASE_URL` (default `http://localhost:8080/v1`) and `LLAMACPP_MODEL` (default `gemma-2-2b-it-q4_k_m`).
- **NPC chat has a server-wide hourly budget** (`NPC_CHATS_PER_HOUR` in `npc_chat.py`, skipped for `llamacpp`). When exhausted, NPCs fall back to static dialog. The system prompt is split into static (per-NPC, cached) and dynamic (per-player) parts for API prompt caching. Cooldown starts from NPC response time, not player message time.
