...
- **AI-powered NPC chat** — talk to NPCs and they respond in character using LLM backends (Claude CLI, Anthropic API, or local Ollama). NPCs can give gifts, summon guards, and react to player behavior through a forced-choice classification system
...
| `ollama` | Install [Ollama](https://ollama.com), pull `gemma2:2b` | Fully local, no API key needed |
...
Deployed on a Hetzner CX22 VPS (Ubuntu 24.04) as a systemd service. Ollama runs locally for NPC chat. See [CLAUDE.md](CLAUDE.md) for deployment details.
