...
- **Command queue architecture**: Websocket messages are never processed inline. The handler just appends `(msg_type, data)` to `player.command_queue` (a deque). The unified `game_tick()` loop drains all queues via `process_player_commands()` before ticking walks/monsters/projectiles. All command processors are sync and append to the `msgs` batch. `ping` is the only message handled directly (latency measurement). Command processors live in `server/commands.py`.
...
- **Movement uses server-side walk state**: walks are continuous (250ms per tile for players), not instant teleports. Server tracks `Player.walk` dict with origin, target, start_time, committed flag. The unified `game_tick()` loop drains command queues (walk/cancel/face/attack/chat), then handles midway commit (position + collision) and walk completion for both players and monsters via `_tick_players()` and `_tick_all_monsters()`. Client sends `walk` (with origin), `cancel_walk`, and `face` messages — all queued and processed next tick. Server responds with `reconcile` (full state snapshot) on any disagreement. Cancel window is 90ms; `LATENCY_COMP=66ms` is the leeway constant. See `docs/PLAN_WALK_SYSTEM.md` for the full design.
