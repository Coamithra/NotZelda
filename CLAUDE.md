...
- **NES Zelda-style half-tile free movement**: players move continuously while keys are held, at `PLAYER_SPEED=4.0` tiles/sec. Positions are sub-tile floats. Client does local collision prediction and sends `position_update` messages at half-tile boundaries. Server validates walkability, anti-cheat (rate + distance), and relays `player_walk_half` to other clients. Other players see smooth half-tile step animations (125ms per step). Axis alignment: switching direction smoothly snaps the perpendicular coordinate to the nearest half-tile. Wall overlap: moving UP lets the top half of the sprite overlap wall tiles (NES Zelda style). Player has `self.x/y` (floats), `self.last_pos_update_time`, `self.last_reported_x/y`. Client state: `G.preciseX/Y` (local float position), `G.lastReportedX/Y` (last sent to server). `_is_position_walkable()` checks all tiles a 1x1 hitbox overlaps; when direction='up' only checks bottom half. All collisions (monsters, hearts, items, guards, projectiles) use float AABB overlap. Monsters still use the old tile-to-tile walk system (`WALK_TIME` for monster walks).
