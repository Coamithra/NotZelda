...
- **Item & key placement pipeline**: placement order is lantern → keys → map/compass. All share an `occupied_cells` set to soft-enforce max 1 item/key per room (deprioritized via sort, never hard-filtered — degenerate layouts gracefully co-locate). Keys prefer rooms off the critical path (entrance→treasure shortest path), in trap rooms, with no other items. Map and compass use zone-aware scoring: BFS on the zone graph to find zones furthest from both entrance and boss zones, with available-cell count as tiebreaker. `_place_dungeon_items()` in `dungeons.py` handles map/compass; key placement is in `_place_locked_doors()`.
