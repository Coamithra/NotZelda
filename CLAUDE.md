...
- **Player position updates**: client sends precise float position every frame (throttled to ~30fps via `POSITION_REPORT_INTERVAL`). Server accepts any float position (no grid-snap validation). Client-side axis-alignment still snaps toward half-tiles for NES Zelda feel.
