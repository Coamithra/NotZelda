...
- **`sprites.js`** — Player & NPC sprite drawing. Procedural 16x16 character rendering with directional poses and walk animation. Exposes `drawPlayer()`, `drawPlayerDance()`, `drawPlayerAttack()`, `drawSwordAttack()`, `drawNPC()` (dispatcher for 14 sprite types), `drawGuard()`, `drawSmith()`, `drawPriest()`, `drawBarmaid()`, `drawAmara()`, `drawWitch()`, `drawGhost()`, `drawGhostKnight()`, `drawRanger()`, `drawFarmer()`, `drawNomad()`, `drawMerchant()`, `drawElder()`, `drawFisher()`, `drawSlime()`/`drawSlimeDeath()`, `drawBat()`/`drawBatDeath()`, `drawScorpion()`/`drawScorpionDeath()`, `drawSkeleton()`/`drawSkeletonDeath()`, `drawSwampBlob()`/`drawSwampBlobDeath()`, `drawHeart()`, `drawHeartPickup()`, `drawPlayerFallOver()`.
- **`tiles.js`** — Tile rendering. Procedural tile textures (grass, stone, wood, water, dungeon wall/floor, etc.) cached to offscreen canvases. Exposes `getTileCanvas(tileId, TS, TILE, SCALE)` and `TILE_COLORS`.
