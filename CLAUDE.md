...
- **Sword hitbox**: 1.5 tiles in the attack direction (extends 0.5 tiles back into the player's own tile), 1 tile perpendicular. This lets players hit monsters overlapping their position. The hitbox stays active for `SWORD_ACTIVE_DURATION` (200ms, ~6 ticks) after the swing — `sword_hit_scan()` in `commands.py` runs each tick, tracking already-hit monsters by `id(monster)` so each is only damaged once per swing. `player.active_attack` holds the swing state; `_tick_active_attacks()` in `combat.py` drives the per-tick scans. The hitbox follows the player's current position (not anchored to where they stood when they pressed attack).
