...
- **Mobile layout**: `scaleForMobile()` in `client/client.html` toggles `body.landscape` on orientationchange/resize. Portrait reserves 180px for bottom controls (virtual joystick bottom-left, action buttons stacked right). Landscape overlays the joystick bottom-left and action buttons bottom-right on the canvas corners (no vertical reserve), so the canvas can use the full viewport height.
