...
- **`server/dungeons.py`** — `DungeonInstance` class (with `cell_assignments`, `resolved_rooms` for Stage 7 lazy resolution), `create_dungeon()` (library-managed, ~50/50 precreated/custom split, only resolves entrance), `resolve_dungeon_room()` (materializes a library entry into `game.rooms[]`), `destroy_dungeon()` (with library expiry + persistence), `is_dungeon_room()`, `dungeon_player_count()`.
...
- Server → Client: `login_ok` (color_index, hp, max_hp), `room_enter` (tilemap + players + guards + monsters + exits + biome + music + exit_direction + hp + max_hp + optional custom_sprites + custom_death_sprites + custom_tiles + optional dungeon_debug), `room_generating` (sent before lazy dungeon room resolution), `player_moved`, `player_entered`, `player_left`, `attack` (name, direction), `chat`, `dance`, `info`, `error`, `pong`, `monster_moved`, `monster_killed`, `monster_hit`, `monster_spawned`, `player_hurt` (name, hp, max_hp, x, y, knockback), `you_died` (x, y), `player_died` (name, x, y, color_index), `hp_update` (hp, max_hp), `heart_spawned` (id, x, y), `heart_collected` (id)
...
**Dungeons:** Library-managed dungeon instances (Stage 7), entered via stairs down in the Clearing room. Each instance uses a random layout from `dungeon_layouts.py` (11 Zelda-inspired 8x8 grid shapes). Cells are assigned content from `game.room_library` with ~50% precreated (permanent library entries from `rooms/dungeon1/` templates) and ~50% custom (AI-generated or placeholder). **Lazy resolution**: only the entrance room is resolved at creation time; other rooms are materialized into `game.rooms[]` when a player first enters them. Custom rooms trigger a "conjuring" animation on the client (1.5s minimum) while the room is being resolved. `DungeonInstance` tracks `cell_assignments` (source, entry, resolved status) and `resolved_rooms`. All dungeon content (4 monsters, 7 tiles) is registered in game registries at startup so `send_room_enter()` sends `custom_sprites`/`custom_tiles` to clients. Dungeon tilemaps use string tile codes (`"DW"`, `"DF"`, etc.) not numeric IDs. Exits between dungeon rooms are auto-generated from grid adjacency; unused exits are walled off. The entrance room gets stairs up back to the Clearing. Each instance gets a random dungeon music track (1 of 7). Dungeon rooms use the `dungeon` biome with 7 tile types: dungeon_wall (DW), dungeon_floor (DF), pillar (PL), sconce_wall (SC), brazier (BZ), mosaic_floor (MF), cracked_floor (CF). Cleared rooms (all monsters killed) don't respawn. On dungeon destruction, oldest custom library entries are expired (10% rate, 24h min age), expired content is cleaned from game registries, and libraries are persisted to `data/*.json`. The `room_enter` message includes `dungeon_debug` for dungeon rooms (library sizes, room source). A new instance is created on demand when anyone enters.
