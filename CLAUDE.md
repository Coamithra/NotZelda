...
- **Half-tile monster movement**: monsters move in `MOVE_STEP` (0.5) tile steps, chained x2 per decision for 1-tile total movement. `_is_walkable_at()` in `behavior_engine.py` checks all tiles covered by a float-position footprint. Monster positions can rest at half-tile boundaries (e.g. 3.5). Knockback and error recovery snap to nearest half-tile (`round(x * 2) / 2`).
...
- **Combat constants tuned**: `ATTACK_COOLDOWN` = 0.27s (was 0.3), `COLLISION_GRACE_PERIOD` = 0.04s (~1 tick). `SWORD_PERP_WIDTH` = 0.6 (was implicit 1.0). `MOVE_STEP` = 0.5 (half-tile monster movement).
