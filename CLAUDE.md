...
- **Multi-dungeon architecture**: Multiple dungeon types supported via `server/dungeon_types.py` `DUNGEON_TYPES` dict. Each type has its own layouts, music, boss tracks, biome, exit room, and content libraries. State: `game.active_dungeons` (type_id → DungeonInstance), `game.room_to_dungeon` (room_id → type_id for O(1) lookup), `game.content_libraries` (type_id → {rooms, monsters, tiles}), `game.deprecated_content` (type_id → {monsters, tiles}). Use `get_dungeon_for_room(room_id)` to find the instance for any room. Room IDs are `{type_id}_{col}_{row}` (e.g. `d1_3_3`, `d2_0_1`). Dungeon entrance exits in `.room` files map to types via `ENTRANCE_TO_TYPE`. Content libraries are per-type with files at `data/{type_id}_*_library.json`.
